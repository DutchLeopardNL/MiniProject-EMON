...
ENUM_EVENTTWO eventTwo = EVENTTWO_STATE_EXECUTED;
unsigned int timer;
...
void FSMRaiseEventTwo (ENUM_EVENTTWO new_event) {
    ENUM_STATE new_state = fsm_state_transition[state][new_event].nextState;
    if ( new_state != STATE_INVALID_TRANSITION ) {
...
void setup() {
  ...
  pinMode(D5,OUTPUT); //YELLOW
  pinMode(D6,OUTPUT); //GREEN

  pinMode(D7, INPUT_PULLUP); // Button
  pinMode(D4, INPUT_PULLUP); // Button
  ...
  // Access to Internet
  WiFi.begin("devolo-bcf2afde08e2", "TPUHHEFJCIJWLAXS"); // Connect with the Wi-Fi
  Serial.print("Starting Connection to Wi-Fi");
...
void red_preTwo() {
    ...
    Serial.println("red_preTwo");
    digitalWrite(D0, LOW);
}

void red_loop() {
  if (!buttonOnePressed && digitalRead(D7) == LOW ) {
    FSMRaiseEvent(EVENT_BUTTON);
    buttonOnePressed = true;
  }
  if(!buttonTwoPressed && digitalRead(D4) == LOW) {
    FSMRaiseEvent(EVENT_BUTTON);
...
void orange_loopTwo() {
    //Serial.println("orange_loop");
    if( (millis() - timer) > 2000 ) {
      FSMRaiseEventTwo(EVENTTWO_TIMER);
    }
}
...
void orange_postTwo() {
    Serial.println("orange_postTwo");
    digitalWrite(D5, HIGH);
}

void green_pre() {
    Serial.println("green_preTwo");
    timer = millis();
    digitalWrite(D3, LOW);
}
...
void outoforder_preTwo() {
    Serial.println("outoforder_preTwo");
    timer = millis();
}
...
void outoforder_postTwo() {
    Serial.println("outoforder_postTwo");
    digitalWrite(D5, HIGH);
}
