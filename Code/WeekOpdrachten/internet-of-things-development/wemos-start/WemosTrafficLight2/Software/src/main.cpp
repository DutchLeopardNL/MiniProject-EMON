...
void connectMQTT() {
  while (!mqtt.connected()) {
    mqtt.connect("iwsn-wemos-client-monkey");
    mqtt.subscribe("iwsn-wemos-event-monkey");
    if ( mqtt.connected() ) {
      Serial.println("MQTT CONNECTED!");
      payload = "{ \"id\": \"monkey\", \"mqtt\": \"iwsn-wemos-client-monkey\", \"ip\": \"" + WiFi.localIP().toString() + "\"}";
      mqtt.publish("iwsn-wemos-monkey", payload.c_str());
...
void setup() {
  ...
  WiFi.begin("devolo-bcf2afde08e2", "TPUHHEFJCIJWLAXS"); // Connect to the Wi-Fi (if not known use WifiManager from tzapu!)
...
void loop() {   
  ...
  if ( millis() - timerMQTT > 5000 ) { // Sent every five second the loop timing of the wemos device
    payload = "{ \"id\": \"monkey\", \"looptiming\": " + String(fsm.getLoopTime()) + "}";
    mqtt.publish("iwsn-wemos-monkey", payload.c_str());
    timerMQTT = millis();
  }
...
void preLight1Green() {
  ...
  digitalWrite(LIGHT_2_ORANGE, HIGH);
  digitalWrite(LIGHT_2_GREEN,  HIGH);
  payload = "{ \"id\": \"monkey\", \"state\": \"light1 green\" }";
  mqtt.publish("iwsn-wemos-monkey", payload.c_str());
  timer = millis();
}
...
void preLight1Orange() {
  ...
  digitalWrite(LIGHT_2_ORANGE, HIGH);
  digitalWrite(LIGHT_2_GREEN,  HIGH);
  payload = "{ \"id\": \"monkey\", \"state\": \"light1 orange\" }";
  mqtt.publish("iwsn-wemos-monkey", payload.c_str());
  timer = millis();
}
...
void preLight1Red() {
  ...
  digitalWrite(LIGHT_2_ORANGE, HIGH);
  digitalWrite(LIGHT_2_GREEN,  HIGH);
  payload = "{ \"id\": \"monkey\", \"state\": \"light1 red\" }";
  mqtt.publish("iwsn-wemos-monkey", payload.c_str());
  timer = millis();
}
...
void preLight2Green() {
  ...
  payload = "{ \"id\": \"monkey\", \"state\": \"light2 green\" }";
  mqtt.publish("iwsn-wemos-monkey", payload.c_str());
  timer = millis();
}
...
void preLight2Orange() {
  ...
  digitalWrite(LIGHT_2_GREEN,  HIGH);
  payload = "{ \"id\": \"monkey\", \"state\": \"light2 orange\" }";
  mqtt.publish("iwsn-wemos-monkey", payload.c_str());
  timer = millis();
}
...
void preLight2Red() {
  ...
  digitalWrite(LIGHT_2_ORANGE, HIGH);
  digitalWrite(LIGHT_2_GREEN,  HIGH);
  payload = "{ \"id\": \"monkey\", \"state\": \"light2 red\" }";
  mqtt.publish("iwsn-wemos-monkey", payload.c_str());
  timer = millis();
}
...
void preOutOfOrder() {
  ...
  digitalWrite(LIGHT_2_ORANGE, LOW);
  digitalWrite(LIGHT_2_GREEN,  HIGH);
  payload = "{ \"id\": \"monkey\", \"state\": \"out of order\" }";
  mqtt.publish("iwsn-wemos-monkey", payload.c_str());
  timer = millis();
}
